...
double distance_proj_plane)
{
	...
	if (t_spr->sprite_top_y < 0)
		t_spr->sprite_top_y = 0;
	// Sprite bottom Y
	t_spr->sprite_bottom_y = (vars->mi.win_height / 2) + \
	(t_spr->sprite_height / 2);
	if (t_spr->sprite_bottom_y > vars->mi.win_height)
		t_spr->sprite_bottom_y = vars->mi.win_height;
...
void	process_sprite(t_vars *vars)
{
	...
	visible_sprites = malloc(sizeof(t_sprite) * vars->mi.sprite);
	if (!visible_sprites)
		exit (0);
